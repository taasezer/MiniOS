...
void shell_main_c(void){
    console_write_str("\nUltimate Shell v4 - COMPLETE\n> ");
    char buf[128]; int idx=0;
    char tmp[512];
    while(1){
        int c = kb_buffer_pop_nonblock();
        if(c==0){ asm volatile("hlt"); continue; }
        if(c==10){ buf[idx]=0; if(idx>0){ if(strcmp(buf,"ls")==0){ int n=ramdisk_ls(tmp,512); tmp[n]=0; console_write_str(tmp);} else if(strncmp(buf,"cat ",4)==0){ ramdisk_cat(buf+4,tmp,512); console_write_str(tmp);} else { console_write_str("Unknown\n"); } } idx=0; console_write_str("\n> "); continue; }
